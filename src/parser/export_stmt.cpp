...
ptree ExportStmt::ToPropertyTree() const {
  ptree export_stmt;
  if (!table_name_.empty()) {
    export_stmt.put(STR_TABLE_NAME, table_name_);
  }
  if (!opt_column_name_list_.empty()) {
    ...
    export_stmt.put_child(STR_COLUMN_NAME_LIST, column_name_list);
  }
  if (!file_path_.empty()) {
    export_stmt.put(STR_FILE_PATH, file_path_);
  }
  return export_stmt;
}
